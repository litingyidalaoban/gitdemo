...
class Config:
    ...
    def __init__(
        ...
        #确保输出的这个值的字典是一个字典，而且不为None
        #前面的处理就是先执行函数再config。
        #得到的就是一个关于参数的字典，cfg_dict然后是一个文件名带py文件内容的dict_text
        if cfg_dict is None:
            cfg_dict = dict()
        elif not isinstance(cfg_dict, dict):
            raise TypeError('cfg_dict must be a dict, but '
                            f'got {type(cfg_dict)}')
        #判断这些key是不是和保留的关键字冲突了
        for key in cfg_dict:
        ...
        if lazy_import is False or \
           lazy_import is None and not Config._is_lazy_import(filename):
            cfg_dict, cfg_text, env_variables = Config._file2dict(
                ...
                lazy_import)
            #把那些config里的东西加到系统变量，然后给他import进来。
    ...
    def diff(cfg1: Union[str, 'Config'], cfg2: Union[str, 'Config']) -> str:
        ...
        return capture.get()
#判断是不是py文件结尾的，然后打开文件，编码为utf-8，读进来所有的代码。
    @staticmethod
